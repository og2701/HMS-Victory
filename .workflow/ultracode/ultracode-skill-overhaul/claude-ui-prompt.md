...
2. Overview
   ...
   - A blocked run keeps the last meaningful step from its phase and shows `Blocked` in the status/banner. For `blocked` + `awaiting_input`, use `Build` when any packet, agent, or check has started or an earlier event records execution/integration; otherwise use `Plan`. Recovery maps to `Verify`. A cancelled run maps to `Done` but remains visibly labelled `Cancelled`, never `Complete`.
