...
class WagerDecisionView(discord.ui.View):
    ...
    async def resolve(self, interaction: Interaction, winner_id: int | None):
        ...
        if winner_id is None:
            # Draw - refund both
            add_bb(self.challenger_id, self.amount, reason="Wager cancelled/refund", taxable=False)
...
class WagerProposalView(discord.ui.View):
    ...
    async def btn_accept(self, interaction: Interaction, button: discord.ui.Button):
        ...
        if not cabinet_channel:
            # Fallback
            add_bb(self.challenger.id, self.amount, reason="Wager declined/refund", taxable=False)
            add_bb(self.opponent.id, self.amount, reason="Wager declined/refund", taxable=False)
