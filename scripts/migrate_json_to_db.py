#!/usr/bin/env python3
"""
Migrate shut_counts.json, warden_targets.json, and roast_targets.json into the SQLite database.
...
def migrate_roast_targets():
    print("📦 Migrating roast_targets.json ...")
    data = load_json(ROAST_TARGETS_FILE)
    if data is None:
        return
    ...
    for user_id, count in data.items():
        c.execute(
            "INSERT INTO roast_targets (user_id, count) VALUES (?, ?) "
            "ON CONFLICT(user_id) DO UPDATE SET count = count + ?",
            (str(user_id), int(count), int(count))
        )
        migrated += 1
    conn.commit()
    print(f"  ✅ Migrated {migrated} roast target entries.")
...
def main():
    ...
    rt = DatabaseManager.fetch_one("SELECT COUNT(*) FROM roast_targets")
    print(f"  shut_counts rows:    {sc[0]}")
    print(f"  warden_targets rows: {wt[0]}")
    print(f"  roast_targets rows:  {rt[0]}")
    print()
    ...
    print(f"  mv {ROAST_TARGETS_FILE} {ROAST_TARGETS_FILE}.bak")
