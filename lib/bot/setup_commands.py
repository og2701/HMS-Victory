...
def define_commands(tree, client):
    ...
    @command("whatsgoingon", "Catch up on the last 10 minutes of chat - screen-reader friendly summary (only you see it)")
    async def whatsgoingon_command(interaction: Interaction):
        await whatsgoingon(interaction)

    @command("wgo", "Short for /whatsgoingon - summary of the last 10 minutes of chat (only you see it)")
    async def wgo_command(interaction: Interaction):
        await whatsgoingon(interaction)
    ...
    @command("anti-raid", "Open the private anti-raid control centre", checks=[lambda i: has_any_role(i, [ROLES.MINISTER, ROLES.CABINET, ROLES.BORDER_FORCE, ROLES.DEPUTY_MINISTER_OF_COMMUNITY])])
    ...
    @command("flags", "Review and lift anti-cheat flags (Oggers only)",
             checks=[lambda i: i.user.id == USERS.OGGERS])
    async def flags_command(interaction: Interaction):
        """Interactive panel. The review buttons on an alert disable themselves once one is
        pressed, so an alert cannot take back its own verdict - and a flagged member is
        blocked from /pay. This is the way back."""
        from lib.core import detection as _D
        content, view = _D.build_flags_panel(interaction.client, interaction.user.id)
        await interaction.response.send_message(
    ...
    @command("glassbridge", "Cross the glass bridge - one panel holds, one shatters (UKPence)")
    async def glassbridge_command(interaction: Interaction, amount: app_commands.Range[int, 1]):
        if await _require_casino_channel(interaction):
            return
        await handle_glass_command(interaction, amount)

    @command("blockade", "Run the blockade - bank the climbing multiplier before they sink you (UKPence)")
    async def blockade_command(interaction: Interaction, amount: app_commands.Range[int, 1]):
        if await _require_casino_channel(interaction):
            return
        await handle_blockade_command(interaction, amount)

    @command("darts", "Throw up to 3 darts - stand on a high score without going bust (UKPence)")
    async def darts_command(interaction: Interaction, amount: app_commands.Range[int, 1]):
        if await _require_casino_channel(interaction):
            return
        await handle_darts_command(interaction, amount)
    ...
    async def penalty_command(interaction: Interaction, amount: app_commands.Range[int, 1]):
        if await _require_casino_channel(interaction):
            return
        await handle_penalty_command(interaction, amount)
    ...
    async def battleship_command(interaction: Interaction, opponent: Member,
        ...
        if await _require_casino_channel(interaction):
            return
        from commands.economy.battleship import handle_battleship_command
        await handle_battleship_command(interaction, opponent, bet)
    ...
    # --- Big Brother (temporary event; gated on config.BIG_BROTHER_ENABLED) ---
