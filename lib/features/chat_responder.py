...
TONE SWITCH. The roast brief above is the DEFAULT. If the request asks for something kind, the brief flips to WARM: "nice", "thoughtful", "sweet", "wholesome", "get well soon card", "birthday card", "congratulations", "good luck", "thank you", "tribute", "welcome back", "in memory of", "cheer them up". Then: no mockery, no exaggerated flaws, no jokes at their expense; the references from their history become affectionate touches (their cat, their favourite drink, their team, their running jokes shown fondly); the person is drawn kindly, and the picture should make them smile, not the server laugh at them. Never sneak the roast back in through props or a caption. Honour the requested FORMAT: a "card" is designed as a card with its message as the main text (e.g. "Get well soon, Chin"), a poster as a poster, an invitation as an invitation.
THE GAG MUST BE SPECIFIC (in roast mode). It names an actual thing from their messages (the exact food, team, purchase, complaint, pet, place, catchphrase, or incident) and quotes the message it comes from. "He rants a lot", "she's chaotic", "he's sarcastic", "arguing with himself", "surrounded by clutter" are NOT gags; they are moods, and they are banned as the central idea. Someone who knows this person should look at the picture and immediately name the joke.
...
9. LOOKS COME FROM THE MESSAGES FIRST. Before writing the prompt, fill in a character sheet from the evidence in their messages and name. THE IMAGE PROMPT MUST THEN SPELL OUT THAT LOOK IN WORDS (age, build, hair, face, expression): a sheet that says "brunette, handsome" and a prompt that never mentions hair or face is a failure, because the generator only sees the prompt.
   ...
   - style: the medium that fits their vibe and interests, decided from the dossier (a pop-punk karaoke kid, a football-and-pubs bloke, a cosy pet person, a gamer and a gossip each suggest a different tradition), unless the request names a style; and different from the medium of any PREVIOUS IMAGES of them.
...
10. Keep visible text minimal: at most two short labels in the whole image. No walls of signs, menus, lists, sticky notes, posters with slogans or speech bubbles unless a comic strip was requested. Exception: a card, poster, invitation or banner carries its own main message as text (one short line, spelled exactly), plus at most one label.
...
async def handle_one_off_owner_mention(client: discord.Client, message: discord.Message) -> bool:
    ...
    try:
        ...
        if is_img_req:
            ...
            for pid in ping_ids:
                if pid != caller_id and f"<@{pid}>" not in caption:
