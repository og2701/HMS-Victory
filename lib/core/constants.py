...
FLAG_LANGUAGE_MAPPINGS = {
    ...
    "🎭": "Theatrical soliloquy - rewrite the message as one or two sentences of the most absurdly overblown melodrama imaginable, wailed alone on a stage to nobody. HARD LIMIT: two sentences AND 35 words in total, never more. That limit is not negotiable - pack the drama in tighter rather than running long. Pitch it as a tragic hero's dying words: invoke fate, the gods, the heavens, the void, ruin, the indifferent stars, the cruelty of existence itself. Stagger, clutch, tremble, sink to your knees, tear at the sky. The subject is always something utterly trivial, and the gap between how small it is and how much you are suffering IS the joke, so never undersell it - if it does not sound ridiculous, it is not dramatic enough. No calm conclusions, no wry observations, no self-awareness, no winking at the audience. Modern English, not Shakespeare, so no thee, thou, hath or forsooth, though 'O' is very welcome. Do not use em-dashes.",
