...
async def on_ready(client, tree, scheduler):
    ...
    scheduler.add_job(
        ...
        kwargs={"date": (datetime.now(pytz.timezone("Europe/London")) - timedelta(days=1)).strftime("%Y-%m-%d")}
    )
    
    # Scheduler for weekly summary
    scheduler.add_job(
    ...
    # Scheduler for clearing image cache
