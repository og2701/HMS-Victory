...
class TestScreenshotCanvas(unittest.TestCase):
    ...
    async def test_screenshotCanvas_sends_initial_embed(
        self, mock_File, mock_Embed, mock_capture
    ):
        interaction = Mock()
    ...
    @patch("commands.canvas.screenshot.Embed")
    @patch("commands.canvas.screenshot.File")
    async def test_screenshotCanvas_calls_capture_screenshot(
        self, mock_File, mock_Embed, mock_capture
    ):
        interaction = Mock()
        await screenshotCanvas(interaction)
    ...
    @patch(
        "commands.canvas.screenshot.capture_screenshot", return_value="screenshot.png"
    )
    @patch("commands.canvas.screenshot.Embed")
    @patch("commands.canvas.screenshot.File")
    async def test_screenshotCanvas_sends_screenshot_file(
        self, mock_File, mock_Embed, mock_capture
    ):
        interaction = Mock()
        await screenshotCanvas(interaction)
